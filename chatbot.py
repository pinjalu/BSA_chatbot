"""RAG chatbot over the bsa-manuals Pinecone index.

Given a question, this script:
  1. Embeds it with text-embedding-3-large (3072 dims, same as ingestion)
  2. Queries Pinecone for top-K most similar chunks
  3. Optionally filters by vehicle_model and/or doc_type
  4. Asks gpt-4o-mini to answer using ONLY those chunks (plus the
     in-memory history of the current user session for follow-ups)
  5. Prints the answer with section + page citations and image refs
  6. Appends the Q&A to chat_logs/<user>.txt for manual review

USAGE
  python chatbot.py "How do I change engine oil on my Bantam?"
  python chatbot.py                                 # interactive REPL
  python chatbot.py --user alice                    # per-user session + log
  python chatbot.py --user alice --resume 5         # rehydrate last 5 turns
  python chatbot.py "..." --vehicle "BSA Bantam"    # restrict to one bike
  python chatbot.py "..." --doc-type owners_manual  # restrict to one doc kind
  python chatbot.py "..." --top-k 8                 # more chunks per question
  python chatbot.py "..." --show-text               # also dump chunk bodies
  python chatbot.py "..." --index bsa-manuals       # different index

REPL commands:
  /clear     wipe in-memory history (keeps the log file)
  /history   show the in-memory history for this session
  exit|quit  leave
...
# Relevance judge: a second, cheap LLM call that runs after the main answer
# is generated and decides whether it actually addresses the user's question.
...
def _gpt5_extra(model: str, intent: str | None = None) -> dict:
    """Return reasoning_effort + verbosity kwargs only for gpt-5
    models; other models reject these fields.

    Settings are read from the IntentProfile in accuracy.py — no
    hardcoded if-chain here. To change the verbosity for a given
    intent, edit the corresponding IntentProfile in accuracy.py.
    Adding a new intent profile is automatically picked up here
    with no code change.
    ...
    if not model.startswith("gpt-5"):
        return {}
    # Read intent-specific generation knobs from the central config.
    # Falls back to the module-level defaults for unknown intents.
    try:
...
def parse_log_history(user: str, last_n: int) -> list[tuple[str, str]]:
    ...
    Used by --resume so a user can continue a prior session. Reads the
    same Q:/A: blocks that append_log writes; if the format changes, this
    will return fewer pairs (or none) but never crash.
    """
    path = log_path_for(user)
    if not path.exists():
        return []
    text = path.read_text(encoding="utf-8", errors="replace")
    # Split on the run of ═ characters that delimit each turn.
    turns = re.split(r"\n=+\n", text.replace("═", "="))
...
# learned-answer lookup + main retrieval using the same query, common
# FAQs across users) hits drop that to ~0 ms.
#
...
def embed_query(client: OpenAI, text: str, use_cache: bool = True) -> list[float]:
    ...
    Cache is keyed on a normalised form of the query (lowercase,
    collapsed whitespace) so trivial wording variations still hit.
...
def embed_cache_stats() -> dict:
    """Snapshot of cache state for diagnostics / logging."""
    return {
...
# retrieval to surface both sides of a comparison or a procedure that
# spans multiple components. Kept tight on purpose; common conjunctions
# like "and"/"or" appear in plenty of single-fact queries and would
# over-trigger the complex tier.
...
def dynamic_top_k(question: str, default: int | None = None) -> int:
    ...
    Cheap heuristic — we'd rather be fast and roughly right than
    train a classifier for 5 ms of latency.

      - Short / single-fact (≤ 6 words)        → 4   (faster LLM)
      - Comparative or long (≥ 15 words / vs)  → 10  (more recall)
      - Everything else                         → `default` (6)

    `default` lets a caller override the medium tier (e.g. when the
    user has explicitly chosen a baseline).
    """
    text = question.strip()
    if not text:
...
def build_filter(vehicle: str | None, doc_type: str | None) -> dict | None:
    ...
    When a vehicle is named, also let through "universal" docs that
    have no vehicle_model field — warranty booklets, brand catalogues,
    SOPs, accessories guides. They apply to all bikes and would
    otherwise be invisible whenever the user mentions a model.
    """
    f: dict = {}
    if vehicle:
        f["$or"] = [
...
def rerank(pc, query: str, matches: list[dict], keep: int = 5,
    ...
    """Re-score Pinecone matches with a cross-encoder and return the
    top `keep`.

    Vector retrieval is fast but coarse — it scores query and document
    independently, then takes the cosine similarity. A cross-encoder
    looks at the query and the chunk *together*, producing a much
    sharper relevance score. Standard upgrade: over-fetch (e.g. 20),
    rerank down to a small precise set (e.g. 5).

    Pinecone hosts `bge-reranker-v2-m3` natively, so no new dependency
    — just an extra API call (~80–200 ms typical).

    Falls back to the original retrieval order if the reranker errors,
    so chat never breaks because of a transient inference failure.
    """
    if not matches:
        return matches
    if keep >= len(matches):
    ...
    try:
        result = pc.inference.rerank(
            ...
            # parts-catalogue or workshop chunks easily exceed this and
            # cause a 400. truncate="END" tells the API to clip the
            # tail of the document instead of erroring.
            parameters={"truncate": "END"},
        )
    except Exception as e:  # noqa: BLE001
        # Don't break the chat path on a rerank failure — surface it
        # in logs and use the bi-encoder ranking we already have.
        import logging
    ...
    for item in result.data:
        ...
        # downstream code (citations, top_score for handoff confidence,
        # image_map) keeps working unchanged.
        reranked.append({
            "id": original.get("id"),
...
def _normalize_chunk_text(text: str) -> str:
    ...
    `Key | Value | Key | Value | ...` separators. Cross-encoders and
    LLMs both score and quote this poorly compared to natural
    language. This pass:

      * splits long pipe-runs onto separate lines
      * collapses runs of whitespace and stray `|` borders

    It deliberately does NOT alter cell contents — torque values,
    part numbers, units, page references stay verbatim, so the
    grounding verifier and citations keep working. Triggers only
    when the chunk is clearly a flattened table (pipe density above a
    line-relative threshold), so prose chunks are passed through
    unchanged.
    """
    if not text or "|" not in text:
        return text
    # Pipe density heuristic — average pipes per line. Real prose
    # rarely exceeds ~0.5; flattened tables run 5+. Threshold avoids
    # touching legitimate prose that happens to use a pipe.
    ...
    for line in lines:
        if line.count("|") < 2:
            ...
            continue
        # Split on pipe runs, drop empty cells (table border artifacts),
        # rejoin with "; " — reads as a sequence of fields rather than
        # a wall of pipes.
    ...
    rebuilt = "\n".join(out_lines)
    # Collapse runs of whitespace introduced by the split.
    import re as _re
...
def render_context(matches: list[dict]) -> str:
    ...
    Image references for each chunk are inlined as "Images: IMG_N, ..."
    using globally unique IDs so the model can emit [[SHOW_IMAGE: IMG_N]]
    tags that the widget resolves to actual <img> elements. Use
    build_image_map(matches) to get the matching id→path map.

    Images are capped at MAX_IMAGES_PER_CHUNK per chunk and
    MAX_IMAGES_IN_CONTEXT total — beyond that the LLM ignores them all
    and we waste tokens on IDs it will never reference.
    """
    blocks = []
    img_id = 0
    total_imgs = 0
...
def build_image_map(matches: list[dict]) -> dict[str, str]:
    ...
    IDs are assigned in the same order as render_context: walk matches in
    order, and for each match's image_paths list assign sequential IDs.
    """
    out: dict[str, str] = {}
    img_id = 0
    for m in matches:
...
def build_chat_messages(question: str, matches: list[dict],
    ...
    the model has conversational continuity for follow-ups like "what
    about the front?" without re-paying for stale context tokens.

    If `image_b64` is supplied (raw base64, no data: prefix), the current
    user turn becomes a multimodal message: text + image_url content
    blocks. The model needs to be vision-capable for this to do anything
    useful (gpt-5-mini, gpt-4o, etc.).
    """
    user_text = (
...
def stream_answer(client: OpenAI, question: str, matches: list[dict],
    ...
    Empty matches are passed through to the LLM (no short-circuit) so the
    prompt's greeting/closing rules can fire on conversational openers.

    `intent` (from the classifier) gates two depth controls: the user-
    message directive (procedure/safety/wiring nudge) and verbosity=high
    on gpt-5 calls. Without it, gpt-5-mini caps procedural output and
    summarizes 8-step procedures into 4.
    """
    directive = _intent_directive(intent)
    augmented_question = directive + question if directive else question
    msgs = build_chat_messages(augmented_question, matches, system_prompt,
...
def detect_vehicle(question: str) -> str | None:
    """Return the Pinecone `vehicle_model` filter value if `question`
    names a known vehicle, else None. Backed by the central VEHICLES
    config in accuracy.py."""
...
def ask(client: OpenAI, index, question: str, args, system_prompt: str,
    ...
    # Auto-detect a vehicle if the user didn't pass --vehicle and the
    # question mentions one. Surfaces results from the right manual without
    ...
    # Classifier: model + variant + intent. Used to (1) bind a filter
    # when the regex didn't catch a model name, (2) tighten/widen top_k
    # based on intent, (3) prefer doc_types relevant to the intent.
    ...
    # Promote a confident classifier vehicle if no explicit/regex hint.
    ...
    # Resolve top_k: an explicit --top-k always wins; otherwise pick
    # dynamically from the question shape, then adjust for intent.
    ...
    # Over-fetch for procedures/safety/wiring so the cross-encoder
    # (when running) can pick from a wider candidate pool. The CLI
    # path doesn't auto-rerank like api.py, so this is a no-op for
    # plain retrieve; kept here so a future --rerank flag would
    # benefit.
    fetch_k = intent_fetch_k(classification, top_k)
    matches = retrieve(index, vec, fetch_k, args.min_score, flt)

    # Drop chunks whose section is in a different subsystem (e.g. a
    # "Fuel Lines" section chunk surviving on a valve-clearance query).
    # Soft filter; preserves a per-intent minimum count.
    matches = filter_by_section_relevance(matches, question, classification)

    # Drop chunks dramatically less relevant than the top match —
    # stops irrelevant content (e.g. fuel-lines chunk on a valve-
    # clearance query) from diluting the LLM context.
    matches = filter_by_rerank_floor(matches, classification)

    # If we over-fetched, trim down to top_k AFTER the floor so
    # diversification has the strongest survivors to pick from.
    if len(matches) > top_k:
        matches = matches[:top_k]

    # Doc-type diversification: round-robin across doc types so
    # multi-source answers see both perspectives.
    ...
    # Page-neighbour expansion (long-form intents only): fetch chunks
    # within ±2 pages of the top match so continuation content (e.g.
    # starter troubleshooting that spans pages 502→503 under a
    # different sub-section) reaches the LLM. Pinecone fetch() ignores
    # IDs that don't exist, so the candidate grid is cheap.
    ...
    # Tier-2 relevance judge: second LLM call that checks whether the answer
    # actually addresses the user's question before it's shown. Runs in a
    # thread with a hard timeout so a slow judge never blocks the chat path.
    ...
    if answer_irrelevant:
        # Skip grounding check — fallback text has no factual atoms to verify.
        print(f"\nAnswer:\n{_RELEVANCE_FALLBACK}")
        ...
        # Tier-1 grounding check (regex). Cheap, no extra LLM call.
        ctx_text = " ".join(

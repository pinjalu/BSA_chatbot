...
function StageDot({ stage, index }) {
  return (
    <div className={`stage-row ${stage.status === 'running' ? 'stage-row--running' : ''}`}>
      <span className={`stage-num stage-num--${stage.status || 'pending'}`}>{index + 1}.</span>
...
export default function Pipeline() {
  ...
  function handleDrop(e) {
    ...
    if (f && f.name.toLowerCase().endsWith('.pdf')) {
      setUploadFile(f);
      setUploadError('');
      setCategory(autoCategory(f.name));
    } else {
        ...
        <div className="pipeline-layout">
          ...
          <div className="pipeline-left">
            ...
            <div className="card upload-card">
              <div className="upload-card__title">Add a new PDF</div>
                ...
                >
                  <input
                    ...
                    onChange={e => {
                      ...
                      if (f) { setUploadFile(f); setUploadError(''); setCategory(autoCategory(f.name)); }
                    }}
                  />
                  <div className="dropzone__empty">
                    <div className="dropzone__icon">📄</div>
                    <div>Drop a PDF here, or <strong className="dropzone__choose">click to choose</strong></div>
                    <div className="dropzone__subtitle">text → images → clean → prepare → embed → live in Pinecone</div>
                  </div>
                </div>
              ) : (
                <div className="confirm-panel">
                  <div className="confirm-panel__title">Ready to process</div>
                  <div className="confirm-panel__file">
                    <span className="confirm-panel__file-emoji">📄</span>
                    <div>
                      <div className="confirm-panel__file-name">{uploadFile.name}</div>
                      <div className="confirm-panel__file-size">
            ...
            <div className="card jobs-card">
              ...
              {jobs.length === 0 ? (
                ...
                <table className="jobs-table">
                  ...
                  <tbody>
                    {jobs.map(job => {
                      const stageList = job.stages ? Object.values(job.stages) : [];
                      const doneCount = stageList.filter(s => s.status === 'done' || s.status === 'skipped').length;
                      const stageStr = stageList.length ? `${doneCount}/${stageList.length}` : '—';

                      let dur = '—';
                      if (job.started_at && job.finished_at) {
                        dur = formatDuration(new Date(job.finished_at) - new Date(job.started_at));
                      ...
                      return (
                        ...
                        >
                          <td className="job-filename" title={job.filename || job.file_name}>
                            {(job.filename || job.file_name || 'Unknown')}
                          </td>
                          <td>{statusBadge(job.status)}</td>
                          <td className="job-stages">{stageStr}</td>
                          <td className="job-time">
                            {job.started_at
                              ? new Date(job.started_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                              : '—'}
                          </td>
                          <td className="job-dur">{dur}</td>
                          <td>
                            <button
                  ...
                  <div className="stages-section">
                    ...
                    {stages.length === 0 ? (
                      ...
                      <div className="stages-list">
                        {stages.map((s, i) => (
                          <StageDot key={s.name || i} stage={s} index={i} />
                        ))}
                      </div>
                    )}

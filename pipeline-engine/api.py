...
_s3_region = (os.environ.get("AWS_DEFAULT_REGION")
              or os.environ.get("AWS_REGION")
              or "ap-southeast-2")
_S3_PRESIGN_TTL = int(os.environ.get("S3_PRESIGN_TTL", "3600"))
...
def _startup() -> None:
    ...
    # We MUST pin the region + endpoint to ap-southeast-2: without them
...
def _parse_s3_url(url: str) -> tuple[str, str] | None:
    """Best-effort parser for both S3 addressing styles:

      virtual-hosted:  https://<bucket>.s3.<region>.amazonaws.com/<key>
      path-style:      https://s3.<region>.amazonaws.com/<bucket>/<key>

    Returns (bucket, key) on success, None if the URL doesn't look like
    an S3 HTTPS URL (e.g. a local relative path stored before the S3 rewrite).
    The key is URL-decoded so spaces and parens come back to the literal
    form expected by S3 GetObject.
    """
...
def _normalize_s3_key(key: str) -> str:
    """Strip stray "Data/" path segments left over from an older indexing run.

    The current S3 bucket layout has no "Data/" folder, but Pinecone records
    upserted before the S3 reshuffle still carry the obsolete prefix
    (e.g. "Workshop Manuals/Data/BSA GOLDSTAR.../images/...").
...
def _sign_image_url(url: str) -> str:
    ...
    # New pipeline stores bare S3 keys like "<stem>/images/<file>".
    # Construct a full HTTPS URL so _parse_s3_url can extract bucket + key.
    if not url.startswith(("http://", "https://")):
        if not _s3_bucket:
            return url
        url = f"https://{_s3_bucket}.s3.{_s3_region}.amazonaws.com/{url}"

    parsed = _parse_s3_url(url)
    if parsed is None:
        return url
    bucket, key = parsed
    key = _normalize_s3_key(key)  # strips stale Data/ segments from old records
    try:
...
class ChatRequest(BaseModel):
    ...
    # message. Discarded after the request.
...
def _filter_procedure_offtopic(
    ...
    Keep chunks that share key terms with the user's question. Query-driven
    (no static feature words). Always protects the top 2 chunks so we don't
    accidentally drop the core evidence on short queries.
    """
...
# A real clarification is typically a 1-3 word reply (a model name, a
# variant, "yes/no") meant to resolve the bot's prior question. A
# multi-word technical question that just happens not to start with a
# question word is NOT a clarification, even if short.
...
# If any of these appear, the message is almost certainly a fresh
# technical question, not a clarification reply to the bot. Matched
# as substrings; lowercase.
...
# its referent" — follow-ups like "what about the rear?" or "and for
# the Scrambler?". For these we keep ONE prior Q/A pair so the model
# knows the topic; everything else gets sent with NO history so the
# model focuses fully on the current question.
...
_GREETING_RX = re.compile(
    r"^\s*("
    r"hi+|helo+|hey+|heya|howdy|"
    r"good\s+(?:morning|afternoon|evening|night|day)|"
    r"how\s+are\s+(?:you|u)\b.*|"
    r"what'?s?\s+up\b.*|wassup|watsup|sup\b|"
    r"hello+|namaste|greetings"
    r")\s*[!?.]*\s*$",
    re.I,
)

_GREETING_RESPONSE = (
    "Hello! 👋 I'm the BSA Motorcycles AI assistant.\n\n"
    "I can help you with:\n"
    "- Service & repair procedures\n"
    "- Parts information\n"
    "- Wiring diagrams\n"
    "- Owner's manual guidance\n"
    "- Diagnostic & SOP steps\n\n"
    "What would you like to know about your BSA?"
)


def _is_greeting(text: str) -> bool:
    return bool(_GREETING_RX.match(text.strip())) and len(text.strip().split()) <= 8


def _stream_static(text: str) -> StreamingResponse:
    def gen():
        yield f"event: token\ndata: {json.dumps({'text': text})}\n\n"
        yield f"event: done\ndata: {json.dumps({'finish_reason': 'stop', 'top_score': 0, 'suggestHandoff': False})}\n\n"
...
def _is_clarification_reply(text: str) -> bool:
    ...
    Guarded against three false-positive cases:
      ...
      3. Messages containing a technical noun → fresh technical question.
...
# when concatenated to a substantive question. Examples: a user replies
# "yes please" to the bot's clarification, the merger glues that onto
# the prior question and embeds the whole thing — the resulting vector
# drifts away from the docs because "yes please" is closer to small-talk
# than to "valve clearance procedure".
...
def _classify_message(text: str) -> str:
    ...
    Order matters: check followup BEFORE clarification, otherwise short
    follow-ups like "And the Scrambler?" get mis-classified as one-word
    clarifications and get merged into the prior question instead of
    paired with history.
    """
...
def _resolve_clarification(
    ...
    """If the user's latest message is a clarification reply, merge it
    with the most recent substantive question and return:

      (retrieval_query, llm_message, pruned_history)

    retrieval_query — clean sentence for embedding/Pinecone lookup.
                      No instructional boilerplate (would dilute the
                      embedding and miss real chunks).
    llm_message     — same content plus a directive telling the model
                      not to re-ask.
    pruned_history  — history with the bot's "ask again" turn(s) removed
                      so the model doesn't pattern-match into looping.

    For non-clarifications or empty history, both query forms are the
    original message.
    """
    ...
    # first occurrence's spelling. Complementary qualifiers (e.g. "Gold
    # Star" then "Euro-5+") stay separate. The LLM's prompt rules tell it
    ...
    # Clean form for retrieval: original question + qualifier(s) only.
    # Also strip pure-affirmative tokens ("yes please", "ok", etc.) —
    # when the user just said "yes" to a clarification, those words are
    ...
    # contains the substantive question + the user's qualifier, which IS
    # the complete intent. Any history here is unrelated older conversation
    # that biases the LLM and pattern-matches into asking again.
...
def _stream_learned_answer(learned: dict, lookup_ms: float) -> StreamingResponse:
    """Stream a saved admin reply through the same SSE protocol the LLM path
    uses, flagged as `learned: true` so the UI can show a 'Saved reply
    from our team' label."""
    ...
    def gen():
        ...
        # Stream in small chunks so the widget's typewriter still feels
        # alive rather than dumping the whole text in one event.
        chunk_size = 12
...
def learn(req: LearnRequest) -> dict:
    ...
    same answer instantly. Each call adds a vector; only the closest
...
def chat(req: ChatRequest) -> StreamingResponse:
    ...
    within ~100ms. Pre-LLM pipeline (classify -> retrieve -> rerank ->
    ...
    # Greetings — return static response instantly, skip all RAG processing
    if _is_greeting(req.message):
        return _stream_static(_GREETING_RESPONSE)

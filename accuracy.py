"""Accuracy layer for the RAG chatbot.

Public API (unchanged callers):
  classify_query / classify_query_async — model + variant + intent
  classifier_to_filter_vehicle          — Pinecone vehicle_model filter
  intent_doc_types / intent_top_k       — per-intent retrieval policy
  intent_fetch_k / intent_min_keep      — over-fetch + floor budget
  filter_by_rerank_floor                — drop weak rerank tail
  filter_by_section_relevance           — drop off-subsystem chunks
  detect_dominant_section / merge_unique_matches — section-locked expansion
  diversify_by_doc_type                 — round-robin across doc_type
  verify_grounding                      — Tier-1 regex hallucination check
  get_executor                          — shared thread pool

Configuration is data-driven, NOT scattered if-chains. To add a new
vehicle, intent, variant, or topic cluster, edit ONE entry in the
config blocks at the top of this module — no changes to the
retrieval/generation logic below.

Env-var overrides for the most-tuned thresholds are honoured:
  ACCURACY_CLASSIFIER_MODEL         - name of the LLM used for routing
  ACCURACY_CLASS_CACHE_MAX          - LRU size for classifier output
  ACCURACY_CLASSIFY_TIMEOUT_S       - max wait for the LLM classifier
  ACCURACY_DEFAULT_REGEX_FLOOR      - rerank-floor used for unknown intents
"""
...
# ═════════════════════════════════════════════════════════════════════
# CONFIGURATION — edit these blocks to extend the system
# ═════════════════════════════════════════════════════════════════════
...
class Vehicle:
    """One known vehicle. Adding a new model = one entry below."""
...
# Single source of truth for all known vehicles. The classifier system
# prompt's enum is generated from this list at startup.
...
@dataclass(frozen=True)
class Variant:
    """One known variant (e.g., emission-standard tier)."""
    id: str
...
# Single source of truth for all per-intent settings.
...
# Topic clusters used by section-relevance filtering. A chunk's
# `section` metadata is "off-subsystem" if it matches a cluster the
# question doesn't, AND the question matches some other cluster.
# Adding a new subsystem = one entry; the filter logic stays the same.
TOPIC_CLUSTERS: dict[str, tuple[str, ...]] = {
    ...
    "cooling":            ("coolant", "radiator", "thermostat", "cooling",
                           ...
                           # these, a question like "my bike overheats"
                           # fails to bind to any cluster and the section
                           # filter is bypassed entirely.
    ...
    "suspension_chassis": ("suspension", "fork", "shock", "swingarm",
                           ...
                           # belongs to this subsystem (wheel balance,
                           # tyre, suspension) — listing here so the
                           # symptom-only question routes correctly.
...
# ═════════════════════════════════════════════════════════════════════
# Derived lookups + helpers (do not edit — driven from config above)
# ═════════════════════════════════════════════════════════════════════

# Build O(1) lookups from the config so each call is constant-time
# regardless of how many vehicles/intents are configured.
...
def _norm_model(raw) -> str:
    """Map any classifier output (or alias) to a canonical Vehicle.id,
    or 'unknown'."""
...
def _norm_variant(raw) -> str:
    """Map any classifier output (or alias) to a canonical Variant.id,
    'any', or 'unknown'."""
    if not isinstance(raw, str):
        return "unknown"
    normed = _normalise_alias(raw)
...
def _norm_intent(raw) -> str:
    """Snap any classifier output to a known intent label, or
    'general' as a safe default."""
...
def _regex_classify(question: str) -> dict:
    ...
    Returns the same shape as classify_query. Used as a fallback when
    the LLM classifier fails or returns low-confidence defaults for a
    query whose surface form is actually unambiguous.
    """
    # Intent — first profile whose regex matches wins. Order in
    # INTENT_PROFILES therefore acts as priority (specific → general).
    intent = "general"
    ...
    # Model — first VEHICLES entry whose pattern matches wins.
    model_hit = "unknown"
    for v in VEHICLES:
    ...
    # Variant — first VARIANTS entry whose pattern matches. List is
    # ordered longest-first so "Euro-5+" beats "Euro-5".
    variant = "unknown"
    for vt in VARIANTS:
...
def classify_query(client: OpenAI, question: str,
    ...
    Diagnostic fields surfaced for debug logging (caller-readable, never
    affect retrieval logic):
      _fallback     : path taken — "llm+regex" | "regex_only" | "regex_only_timeout"
      _llm_raw      : raw JSON string the LLM returned (None on error)
      _llm_error    : LLM error message (only on failure)
      _llm_class    : the LLM-derived signals before merge (None on error)
      _regex_class  : the regex-derived signals (always present)
    """
...
# ═════════════════════════════════════════════════════════════════════
# Per-intent retrieval policy (all read from INTENT_PROFILES)
# ═════════════════════════════════════════════════════════════════════
...
def intent_doc_types(classification: dict) -> list[str]:
    """Preferred doc_types for this intent; empty list = no preference."""
...
def intent_min_keep(classification: dict) -> int:
    """Lower bound on chunks delivered to the LLM regardless of how
    aggressively the rerank floor would trim."""
...
def _classify_topics(text: str) -> set[str]:
    """Return the set of topic clusters whose keywords appear in `text`."""
    if not text:
        return set()
    low = text.lower()
    hits: set[str] = set()
...
def filter_by_section_relevance(matches: list[dict], question: str,
    ...
    """Soft filter: drop chunks whose subsystem differs from the
    question's (e.g. fuel-lines chunk on a valve query). Preserves at
    least `intent_min_keep` chunks even if filtering would trim too
    aggressively.

    Topic detection considers BOTH the chunk's section label AND its
    body text. Many BSA chunks carry an umbrella section name (e.g.
    "Technical Specifications", "General Information") that says
    nothing about the actual subsystem. Falling back to body-text
    keywords lets us surface the real topic instead of dropping the
    chunk because its label is generic.
    """
    if not matches:
        return matches
    ...
    for m in matches:
        ...
        body = md.get("text") or ""
        # Body scan capped to keep it cheap; the first few hundred
        # chars are enough to identify the chunk's dominant subsystem.
...
def detect_dominant_section(matches: list[dict],
    ...
    Two ways a section can be 'dominant' enough to trigger expansion:
      (A) Majority — `min_share` (default 50%) of the top `top_n`
          chunks share the same section. Used when several reranked
          chunks cluster on one workshop section (e.g., SHIM SETTING
          spanning pages 204–208).
      (B) Score gap — the TOP chunk's score is `dominance_ratio`×
          higher than the median of the remaining matches. Used when
          one chunk strongly matches the query and the rest are
          loosely related (e.g., starter troubleshooting on page
          502 scoring 0.961 while related chunks score ~0.30). Without
          this branch, single-strong-match queries miss adjacent
          continuation content because no majority forms.

    When `question` is supplied AND its topic clusters are identifiable,
    the score-gap branch additionally requires that the candidate
    section's name (or top chunk's body) shares a topic cluster with
    the question. Stops the lock from anchoring on a section that
    happens to score high but isn't actually about the user's subject —
    e.g. the reranker ranking 'Engine Misfiring' #1 on an overheating
    question because the misfiring chunk mentions 'overheat' in passing.
    """
    if not matches:
        return None
    ...
    # When the question's topic clusters are identifiable, prefer the
    # section whose name BEST aligns with the question — measured by
    # how many of the question's topic-cluster keywords appear in the
    # section name. The section name is what the lock filters on
    # next, so picking the best topic-aligned section gives the
    # cleanest expansion. Without this guard, the lock can fire on a
    # high-scoring chunk whose section is in the right top-level
    # subsystem but whose leaf name is about an unrelated symptom
    # (e.g. "Radiator & HOSE PIPE > Symptom 10: Misfiring" beating
    # "Radiator & HOSE PIPE > Symptom 1: Overheating" on an overheat
    # question, just because the misfire chunk happened to rerank
    # higher).
    if q_topics:
        # Build the keyword set for the question's topic clusters from
        # TOPIC_CLUSTERS — same data-driven source used everywhere
        # else in this module.
        topic_keywords: set[str] = set()
        for cluster in q_topics:
            for kw in TOPIC_CLUSTERS.get(cluster, ()):
                topic_keywords.add(kw)
        # Also add the question's own non-trivial words so that
        # "overheats" matches a section literally named "Overheating"
        # even when the cluster keyword is just "overheat".
        ...
        for m in matches:
            ...
            # Higher overlap wins; rerank score is the tie-breaker.
    ...
    if len(matches) >= 2:
        ...
        # Use the median of the rest, not the second-best; one
        # accidental near-duplicate shouldn't suppress this signal.
        if rest_scores:
            ...
            if mid > 0 and top_score >= mid * dominance_ratio:
                ...
                if top_section:
                    # If we get here with q_topics set, no section name
                    # matched the question topic in branch (C). Fall
                    # back on body+section topic check before honouring
                    # score-gap dominance — guards against single
                    # high-rerank chunks whose subsystem differs from
                    # the question's.
                    if not q_topics:
                        return top_section
                    section_topics = (
                    ...
                    if section_topics & q_topics:
                        return top_section
                    # else: drop through to majority dominance below.
...
def page_neighbor_ids(top_match: dict, span: int = 2) -> list[str]:
    """Return Pinecone IDs for chunks within ±`span` pages of the
    top match, from the same source PDF. The chunk IDs follow the
    pattern `<source>::p<page>_s<sec>_c<chunk>` (or `_t<idx>` for
    tables), so we can probe a small grid of neighbour candidates
    without an embedding round-trip.

    Generates candidate IDs across:
      - pages in [top_page - span, top_page + span]
      - section indices 1..3
      - chunk slots c1, c2, c3 plus t1, t2

    Pinecone's fetch() ignores IDs that don't exist — over-generating
    is cheap and gets us all the page-continuation chunks the
    rerank-by-score pipeline misses.
    """
...
def merge_unique_matches(primary: list[dict],
    ...
    """Append matches from `secondary` that aren't already in `primary`,
...
def filter_by_rerank_floor(matches: list[dict],
    ...
    The floor suppresses noise; the minimum guarantees the LLM has
    enough context to reproduce a multi-section answer."""
    if not matches:
        return matches
...
def diversify_by_doc_type(matches: list[dict], keep: int,
    ...
    """Re-order/trim matches so the kept top-N spans multiple
    doc_types where possible. Round-robin one pick per bucket; preserves
    rerank order within each bucket."""
...
# ═════════════════════════════════════════════════════════════════════
# Grounding verifier (Tier-1 regex)
# ═════════════════════════════════════════════════════════════════════

# Atomic factual tokens the LLM is most likely to hallucinate. Each
# regex is paired with a "kind" label that surfaces in the unsupported
# list. Add new kinds here.
...
def _normalise_for_match(text: str) -> str:
    """Lower + collapse whitespace + strip common punctuation."""
...
def verify_grounding(answer: str, ctx_text: str,
    ...
    # Build a normalised set of valid page numbers from chunk metadata.
    ...
    for kind, rx in _ATOMIC_TOKENS:
        for m in rx.finditer(answer_body):
            ...
            if _normalise_for_match(val) in norm_ctx:
                continue
            # Page references get a second pass against the metadata
            # page set: "p. 204" → "204" → check.
            if kind == "page_ref":
...
# Public symbol — chatbot.py / api.py import this to detect a vehicle
# from raw question text without having to know the regex internals.
